...
//     // Get total count
//     const countQuery = `SELECT COUNT(*) as total FROM subcategory ${whereClause}`;
...
//       `INSERT INTO subcategory (category_id, name, description, image_url, display_order, status)
//              VALUES (?, ?, ?, ?, ?, ?)`,
//       [
//         category_id,
//         name,
...
//              WHERE id = ?`,
//       [category_id, name, description, image_url, display_order, status, id],
//     );
...
export const getAllSubcategories = async (req, res) => {
  try {
    ...
    const dataQuery = `
            SELECT * FROM subcategory 
            ${whereClause}
            ORDER BY id ASC  -- ✅ Changed from display_order to id
            LIMIT ? OFFSET ?
        `;
    const dataParams = [...params, limit, offset];
    const [rows] = await pool.query(dataQuery, dataParams);
